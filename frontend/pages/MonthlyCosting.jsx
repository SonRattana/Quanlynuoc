...
export default function MonthlyCosting() {
    ...
    // 💡 Lấy năm, tháng, ngày theo đúng giờ địa phương (Việt Nam)
    const y = currentDate.getFullYear();
    const m = String(currentDate.getMonth() + 1).padStart(2, '0'); // Tháng trong JS đếm từ 0 nên phải +1
    const d = String(currentDate.getDate()).padStart(2, '0');

    // 💡 Tự ghép chuỗi YYYY-MM-DD
    ...
    const handleFinalize = async (e) => {
        ...
        const payload = {
            ...
            total_electric: Number(costs.total_electric.replace(/\./g, "")) || 0,
            total_water: Number(costs.total_water.replace(/\./g, "")) || 0,
        ...
        try {
            ...
            setCosts({ total_electric: "", total_water: "", total_labor: "", total_depreciation: "" });
        } catch (err) {
                    ...
                    <div className="col-md-8 mx-auto">
                        <div className="bg-white p-4 rounded-4 shadow-sm border-top border-danger border-4">
                            <form onSubmit={handleFinalize}>
                                ...
                                {/* INPUT CHI PHÍ */}
                                {/* 💡 ĐÃ SỬA: CHIA 2 Ô ĐIỆN VÀ NƯỚC */}
                                <div className="row mb-3">
                                    <div className="col-md-6 mb-3 mb-md-0">
                                        <label className="form-label fw-bold text-primary"><i className="bi bi-lightning-charge-fill me-1"></i> Tổng tiền Điện</label>
                                        <div className="input-group input-group-lg">
                                            <input type="text" className="form-control text-end fw-bold text-primary" placeholder="0"
                                                value={costs.total_electric}
                                                onChange={(e) => handleInputChange("total_electric", e.target.value)} />
                                            <span className="input-group-text bg-light fw-bold text-muted">VNĐ</span>
                                        </div>
                                    </div>

                                    <div className="col-md-6">
                                        <label className="form-label fw-bold text-info"><i className="bi bi-droplet-fill me-1"></i> Tổng tiền Nước</label>
                                        <div className="input-group input-group-lg">
                                            <input type="text" className="form-control text-end fw-bold text-info" placeholder="0"
                                                value={costs.total_water}
                                                onChange={(e) => handleInputChange("total_water", e.target.value)} />
                                            <span className="input-group-text bg-light fw-bold text-muted">VNĐ</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-3">
                                    <label className="form-label fw-bold text-success"><i className="bi bi-people me-1"></i> Tổng tiền lương nhân công</label>
                                    <div className="input-group input-group-lg">
